...
import com.yangyi.graduationproject.dao.*;
import com.yangyi.graduationproject.entities.*;
import com.yangyi.graduationproject.exception.DeleteException;
import com.yangyi.graduationproject.exception.GrantException;
...
public class RoleServiceImpl implements RoleService {
     ...
     */
    @CacheEvict(value = ROLE_CACHE, key = "#role.id")
    @Override
     ...
     * 添加事务，保证中间删除失败时可以回滚
     */
    @CacheEvict(value = {ROLE_CACHE,ROLES_CACHE}, allEntries = true)
    @Transactional
    @Override
    ...
    public Integer grantPermission(String roleId, String newPermissionIds) {
        ...
        if (result == 0) {
            grantException = new GrantException("授权失败，请重试");
            LogUtil.warn(this, "角色授权", "给用户：【" + role.getName() + "】授权失败！原因：修改角色最后更新时间时失败。");
            throw grantException;
        }
        return result;
    }
